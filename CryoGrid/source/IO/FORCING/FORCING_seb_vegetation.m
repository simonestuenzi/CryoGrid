...
%         end
        ...
        function forcing = finalize_init(forcing, tile)
            ...
            forcing.DATA.p = temp.FORCING.data.pressure;
...
%             end
        ...
        function forcing = interpolate_forcing(forcing, tile)
            ...
            if (isnumeric(posit) && isscalar(posit) && floor(posit) == posit && posit > 0) || islogical(posit)
                % 'posit' is a real positive integer or a logical value
                % Your code here for the case when 'posit' is valid
            else
                % 'posit' is not a real positive integer or a logical value
                % Your code here for handling the case when 'posit' is not valid
% % %                 disp('posit');disp(posit);                
% % %                 disp('t');disp(t);
% % %                 disp('timeForcing(1,1)');forcing.DATA.timeForcing(1,1)
% % %                 disp('timeForcing(2,1)');forcing.DATA.timeForcing(2,1)
            end
