...
for p = 1:1 %vegetation.params.npts number of grid points = 1
    ...
    ground.STATVAR.vegetation.canopy.lai_winter(p) = ground.PARA.lai_winter; %4.051612734794617; %5.051612734794617;
    ...
    ground.STATVAR.vegetation.canopy.pai_winter(p) = ground.STATVAR.vegetation.canopy.lai_winter(p) + ground.STATVAR.vegetation.mlcanopyinst.sai; %4; %5; %8.; %5.0;                                           % Leaf area index of canopy (m2/m2)
    lai_inc = 0.5;
    ground.STATVAR.vegetation.mlcanopyinst.nveg_winter(p) = ground.STATVAR.vegetation.mlcanopyinst.nveg_summer(p);
    
%     round(ground.STATVAR.vegetation.canopy.lai_winter(p) / lai_inc);          % Number of leaf layers in canopy

    % Minimum number of layers for Norman radiation
    if (ground.STATVAR.vegetation.mlcanopyinst.nveg_winter(p) < 9)
        ground.STATVAR.vegetation.mlcanopyinst.nveg_winter(p) = 9;
        lai_inc = ground.STATVAR.vegetation.canopy.lai_winter(p) / ground.STATVAR.vegetation.mlcanopyinst.nveg_winter(p);
    end
    
    %Add Sebastian
    veg_layer_thickness = ground.STATVAR.vegetation.mlcanopyinst.ztop(p) ./ (ground.STATVAR.vegetation.mlcanopyinst.nveg_winter(p)+1);
    %End ADD SEBASTIAN
    ...
    ground.STATVAR.vegetation.canopy.ntop_winter(p) = ground.STATVAR.vegetation.canopy.nbot(p) + ground.STATVAR.vegetation.mlcanopyinst.nveg_winter - 1;      % Top leaf layer
    
    % Set LAI of each layer 
    for ic = ground.STATVAR.vegetation.canopy.nbot(p):ground.STATVAR.vegetation.canopy.ntop_winter(p)
    ...
    for ic = ground.STATVAR.vegetation.canopy.ntop_winter(p):-1:ground.STATVAR.vegetation.canopy.nbot(p) %
        if (ic == ground.STATVAR.vegetation.canopy.ntop_winter(p))
    ...
    ic = ground.STATVAR.vegetation.canopy.ntop_winter(p);
    ground.STATVAR.vegetation.mlcanopyinst.zw_winter(p,ic) = ground.STATVAR.vegetation.mlcanopyinst.ztop(p); %ztop(p)
    
    for ic = ground.STATVAR.vegetation.canopy.ntop_winter(p)-1: -1: ground.STATVAR.vegetation.canopy.nbot(p)
        %CHANGES SEBASTIAN
%         ground.STATVAR.vegetation.mlcanopyinst.zw_winter(p,ic) = ground.STATVAR.vegetation.mlcanopyinst.zw_winter(p,ic+1) - lai_inc;
        ...
        ground.STATVAR.vegetation.mlcanopyinst.zw_winter(p,ic) = ground.STATVAR.vegetation.mlcanopyinst.zw_winter(p,ic+1) - veg_layer_thickness;
...
%     lai_inc = dz_to_zref / n_to_zref;
    ...
    %END CHANGE SEBATSIAN
    ground.STATVAR.vegetation.mlcanopyinst.ncan_winter(p) = ground.STATVAR.vegetation.canopy.ntop_winter(p) + n_to_zref;

    
    ic = ground.STATVAR.vegetation.mlcanopyinst.ncan_winter(p);
    ground.STATVAR.vegetation.mlcanopyinst.zw_winter(p,ic) = ground.STATVAR.vegetation.mlcanopyinst.zref(p);
    for ic = ground.STATVAR.vegetation.mlcanopyinst.ncan_winter(p)-1: -1: ground.STATVAR.vegetation.canopy.ntop_winter(p)+1
        %CHANGE SEBASTIAN
        ...
        ground.STATVAR.vegetation.mlcanopyinst.zw_winter(p,ic) = ground.STATVAR.vegetation.mlcanopyinst.zw_winter(p,ic+1) - above_veg_layer_thickness;   
%     ground.STATVAR.vegetation.mlcanopyinst.zw_winter(p,ic) = ground.STATVAR.vegetation.mlcanopyinst.zw_winter(p,ic+1) - lai_inc;                  %zw = Canopy heights at layer interfaces (m)
    ...
    for ic = ground.STATVAR.vegetation.canopy.nbot(p):ground.STATVAR.vegetation.mlcanopyinst.ncan_winter(p)
...
%             ground.STATVAR.vegetation.canopy.dpai(p,ground.STATVAR.vegetation.canopy.nbot) = 0.2; %2; %32;
    ...
    %%% Simone: Why should this be done?? 
    pai_miss = 0;
    for ic = ground.STATVAR.vegetation.canopy.nbot(p):ground.STATVAR.vegetation.canopy.ntop_winter(p)
...
%             ground.STATVAR.vegetation.canopy.dpai_winter(p,ic) = 0;
    ...
    for ic = ground.STATVAR.vegetation.canopy.nbot(p)
        if ground.STATVAR.vegetation.canopy.dpai_winter(p,ic) < 0.035 %here the top leaf layer gets PAI = 0. if < 0.01
            ground.STATVAR.vegetation.canopy.dpai_winter(p,ic) = 0.035;
        end
    end
    ...
    for ic = ground.STATVAR.vegetation.canopy.ntop_winter(p)+1:ground.STATVAR.vegetation.mlcanopyinst.ncan_winter(p)
        ground.STATVAR.vegetation.canopy.dpai_winter(p,ic) = 0;
    end
...
% %     %%%%%%%%%%%%
% %     
% %     ground.STATVAR.vegetation.mlcanopyinst.nveg(p) = ground.STATVAR.vegetation.mlcanopyinst.nveg_winter(p);
% %     ground.STATVAR.vegetation.canopy.ntop(p) = ground.STATVAR.vegetation.canopy.ntop_winter(p);
% %     
% %     % Set canopy LAI, layer LAI increment, and number of layers
