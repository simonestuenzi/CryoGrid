...
for p = 1:1 %vegetation.params.npts number of grid points = 1
    ...
    %Add Sebastian
    veg_layer_thickness = ground.STATVAR.vegetation.mlcanopyinst.ztop(p) ./ (ground.STATVAR.vegetation.mlcanopyinst.nveg_summer(p)+1);
    %End ADD SEBASTIAN
...
%         ground.STATVAR.vegetation.mlcanopyinst.zw_summer(p,ic) = ground.STATVAR.vegetation.mlcanopyinst.zw_summer(p,ic+1) - lai_inc;
        ...
        ground.STATVAR.vegetation.mlcanopyinst.zw_summer(p,ic) = ground.STATVAR.vegetation.mlcanopyinst.zw_summer(p,ic+1) - veg_layer_thickness;
...
%     lai_inc = dz_to_zref / n_to_zref;
    ...
    for ic = ground.STATVAR.vegetation.mlcanopyinst.ncan_summer(p)-1: -1: ground.STATVAR.vegetation.canopy.ntop_summer(p)+1
        ...
        ground.STATVAR.vegetation.mlcanopyinst.zw_summer(p,ic) = ground.STATVAR.vegetation.mlcanopyinst.zw_summer(p,ic+1) - above_veg_layer_thickness;   
...
%             ground.STATVAR.vegetation.canopy.dpai_summer(p,ic) = 0;
    ...
    for ic = ground.STATVAR.vegetation.canopy.nbot(p)
        if ground.STATVAR.vegetation.canopy.dpai_summer(p,ic) < 0.035 %here the top leaf layer gets PAI = 0. if < 0.01
            ground.STATVAR.vegetation.canopy.dpai_summer(p,ic) = 0.035;
        end
    end
