...
%             if isfield(snow.VEGETATION, 'STATVAR')
%             if isfield(snow.VEGETATION.ForcingV, 'TEMP')
%             if isfield(snow, 'VEGETATION') && isfield(snow.VEGETATION, 'ForcingV') && isfield(snow.VEGETATION.ForcingV, 'TEMP')
            try
                if ~isempty(snow.VEGETATION)
...
%             snow.TEMP.wind = tile.FORCING.TEMP.wind;
            snow.TEMP.wind = forcing.TEMP.wind; % SMS: fixed to SNOW_crocus2_bucketW_seb
...
%             if isfield(snow.PARENT, 'VEGETATION') && isfield(snow.PARENT.VEGETATION, 'ForcingV') && isfield(snow.PARENT.VEGETATION.ForcingV, 'TEMP')
%             if isfield(snow.PARENT.VEGETATION.ForcingV, 'TEMP')
            try
                if ~isempty(snow.PARENT.VEGETATION)
...
%             end
            ...
            snow.STATVAR.evaporation = 0; % SMS: maybe change to evaporation?
            snow.TEMP.evaporation_energy = 0; % % SMS: maybe change to evaporation?
...
%             snow.TEMP.wind = FORCING.TEMP.wind;
        ...
        function [snow, S_up] = penetrate_SW(snow, S_down)  %mandatory function when used with class that features SW penetration
            [snow, S_up] = penetrate_SW@SNOW_crocus2_bucketW_seb(snow, S_down);
        end
        
        function [snow, L_up] = penetrate_LW(snow, L_down)
            [snow, L_up] = penetrate_LW@SNOW_crocus2_bucketW_seb(snow, L_down);
        end
